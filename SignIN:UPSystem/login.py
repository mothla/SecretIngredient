...
def signup_command():
    ...
    def signup():
       ...
       if password==conform_password:
           #if file is available then it will read file, and enter data
           try:
               file=open('datasheet.txt','r+')
               d=file.read()
               r=ast.literal_eval(d)
    ...
    Lable(window,image=img,bg='white').place(x=50,y=90)

    frame=Frame(root,width=350,height=390,bg="white")
    frame.place(x=480,y=50)


    heading=Lable(frame,text='Sign up', fg='#F37D0C',bg='white',font=('Microsoft YaHei UI Light',23,'bold'))
    ...
    signin= Button(frame,width=6,text='Sign in',border=0,bg='white',cursor='hand2',fg='#F37D0C',command=sign)
...
img = PhotoImage(file= 'logo.JPG')
Lable(root,image=img,bg='white').place(x=50,y=50)

frame=Frame(root,width=350,height=350,bg="white")
frame.place(x=480,y=70)

heading=Lable(frame,text='Sign in', fg='#F37D0C',bg='white',font=('Microsoft YaHei UI Light',23,'bold'))
heading.place(x=100,y=5)
##########----------------------------------------------
def on_enter(e):
    user.delete(0, 'end')
